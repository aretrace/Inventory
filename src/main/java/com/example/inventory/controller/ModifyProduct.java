...
public class ModifyProduct extends AbstractNavigation implements Initializable, BindingUtils, CustomDialogs {
    ...
    /**
     * [RUNTIME ERROR] Made selectedPartForAssociation static, added set method for reassignment
     */
    private void setSelectedPartForAssociation() {
        if (selectedPartForAssociation.size() == 0) {
            ModifyProduct.selectedPartForAssociation.add(allPartTableView.getSelectionModel().getSelectedItem());
        } else {
            ModifyProduct.selectedPartForAssociation.set(0, allPartTableView.getSelectionModel().getSelectedItem());
        }
    }
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // TODO: when Part fields are modified in the other views, update the tableview
        // rn data columns are stale if change is made to a Part
        associatedPartTableView.setItems(Inventory.lookupProduct(MainPanel.selectedProduct.getId()).getAllAssociatedParts());
    ...
    private void populateTextFields() {
        Product product = Inventory.lookupProduct(MainPanel.selectedProduct.getId());
        idTextField.setText(Integer.toString(MainPanel.selectedProduct.getId()));
        ...
        try {
            ...
            if (!(min < max)) {
                ...
                Product modifiedProduct = new Product(
                        MainPanel.selectedProduct.getId(),
                ...
                Inventory.updateProduct(MainPanel.selectedProduct.getId(), modifiedProduct);
    ...
    public void deleteAssociatedPartHandler() {
        Product currentProduct = Inventory.lookupProduct(MainPanel.selectedProduct.getId());
        ...
        //if (alert.showAndWait().get() == ButtonType.OK) {
            if (currentProduct != null) {
                currentProduct.deleteAssociatedPart(associatedPartTableView.getSelectionModel().getSelectedItem());
                associatedPartTableView.setItems(currentProduct.getAllAssociatedParts());
            }
        //}
    }
}
